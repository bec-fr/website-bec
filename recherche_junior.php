...
<!-- Bar Filter properties-->
							...
							<?
								  ...
								  if(isset($_GET["id_date"]) && $_GET["id_date"] != ""){
									  ...
									  //$query .= " AND fj.id IN (SELECT rid_fiche FROM fiche_junior_date WHERE date_debut >= '".addslashes($_GET["date_depart"])."')";
									  $query .= " AND fj.id IN (SELECT rid_fiche FROM fiche_junior_date WHERE date_debut = '".$data2["date_debut"]."' AND date_fin = '".$data2["date_fin"]."')";
								  }
...
?>											 							 
					     ...
					     <?if($i == 0){
											...
											?>Désolé, aucun séjour ne correspond à votre recherche. Vous pouvez modifier vos critères, ou rechercher un séjour via le menu principal.
<br>Vous avez besoin de conseils ? Les conseillers en séjours linguistiques du BEC sont joignables au <b>01 55 35 25 00</b><?
                    ...
                    <div class="content-carousel">
                        <div class="row">
                            <div class="col-md-12">
                                <!-- Title-->
                                <div class="titles">
                                    <h2>Nos autres séjours <?= $pays_recherche ?></h2>
                                </div>
                                <!-- End Title-->
                            </div>
                        </div>
                        ...
                        <div id="properties-carousel" class="properties-carousel">
                           <?
                                    if ($pays_ref == 3) // EDIT : sejours campus a vendre entierement
                                    {
                                        $query = "SELECT fj . * , p.nom AS pays, v.nom AS ville
                                                    FROM fiche_junior fj
                                                    INNER JOIN junior_pays p ON fj.pays = p.id
                                                    INNER JOIN junior_ville v ON fj.ville = v.id
                                                    WHERE fj.id
                                                    IN ( 25, 63, 53 ) 
                                                    GROUP BY fj.id
                                                    ORDER BY fj.nom";
