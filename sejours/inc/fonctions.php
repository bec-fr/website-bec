...
function connexion (){
	...
	if($_SERVER['HTTP_HOST']=="192.168.1.201"){
		$link = mysql_connect("localhost", "root", "FDrgEEd") or die("Impossible de se connecter : " . mysql_error());	
	}else{
...
function calcul_avoir($total,$code){	
	$reduc=0;
	if($code!=""){
		...
		if(mysql_num_rows($req)>0){
			...
			if($row['date_debut']<=$date && $date<=$row['date_fin']){
				if($total>=$row['prix_mini']){
					...
					echo "<script>alert('Pour utiliser cet avoir il faut que le montant total de votre commande (hors frais de port) soit supérieur à ".$row['prix_mini']." €.');</script>";	
					$_SESSION['code']="";
				}
			}else{
				echo "<script>alert('Cet avoir n\'est plus valide.');</script>";	
				$_SESSION['code']="";
			}
		}else{
			echo "<script>alert('Il n\'existe pas d\'avoir pour ce code.');</script>";
		}
	}
	return $reduc;
}
