...
function sup(id, url_retour) {
 if (confirm("Etes-vous sûr de vouloir supprimer cette demande de brochure ?")) {
  document.location.href='brochure-sup.php?n='+id+'&url_retour='+encodeURIComponent(url_retour);
 }
}
</script>
...
<div align="center"><input type="button" value="Exporter" onclick="window.open('brochure-export.php?<?=$plus_link?>', '')" class="bouton" /></div><br />
...
<tr class="titre3">
    ...
    <th class="cellule_basdroite">Message</th>
...
while($row = mysql_fetch_array($exec))
{
	...
	<td class="cellule_basdroite"><?=stripslashes($row['message'])?> </td>
    ...
    <td class="cellule_basdroite"><a href="admin.php?action=brochureModif&id=<?=$row[0]?>"><img src="../adminImg/crayon.gif" border="0" /></a></td>
	<td class="cellule_basdroite"><a href="javascript:sup('<?=$row[0]?>', '<?=urlencode("admin.php?action=brochure&page=".$page.$plus_link)?>');"><img src="../adminImg/poubelle.gif" border="0" /></a></td>
	</tr>
	<?
}
?>
...
for($i=0 ; $i<ceil($nb/50) ; $i++)
{
	...
	echo ($i==$page ? "<b>".($i+1)."</b>" : "<a href='admin.php?action=brochure&page=$i".$plus_link."'>".($i+1)."</a>");
}
?>
</p>
