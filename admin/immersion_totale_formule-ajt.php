...
if(isset($_POST['nom']) && $_POST['nom'] != ""){
	...
	if(isset($_GET['id'])){
		$requete = "UPDATE formule_immersion_totale SET nom='".addslashes($nom)."', description='".addslashes($description)."' WHERE id = '".addslashes($_GET['id'])."'";
		mysql_query($requete) or die(mysql_error());
		$id = $_GET['id'];
	}else{
		$requete = "INSERT INTO formule_immersion_totale (nom, description) VALUES ('".addslashes($nom)."', '".addslashes($description)."')";
		mysql_query($requete) or die(mysql_error());
		$id = mysql_insert_id();
	}
	...
	if(isset($_GET['id'])){
		$requete = "SELECT * FROM formule_immersion_totale WHERE id = '".addslashes($_GET['id'])."'";
