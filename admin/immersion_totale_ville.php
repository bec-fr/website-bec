...
<input type="hidden" name="action" value="immersion_totale_ville" />
...
<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%" class="tableau">
	<tr class="titre2"><td>Villes</td><td align="right"><a href="admin.php?action=immersion_totale_villeAjouter" style="color:#FFFFFF">Ajouter une ville</a></td></tr>
	<tr>
		<td colspan="2">
<br />
...
function sup(id) {
 if (confirm("Etes-vous sûr de vouloir supprimer cette ville ?")) {
  document.location.href='immersion_totale_ville-sup.php?n='+id;
 }
}
</script>
...
$query = "SELECT v.*, p.nom as pays FROM immersion_totale_ville v LEFT OUTER JOIN immersion_totale_pays p ON v.pays = p.id WHERE 1";
...
while($row = mysql_fetch_array($exec))
{
	...
	<td class="cellule_basdroite"><a href="admin.php?action=immersion_totale_villeModif&id=<?=$row[0]?>"><img src="../adminImg/crayon.gif" border="0" /></a></td>
