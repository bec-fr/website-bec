...
if(isset($_POST['nom']) && $_POST['nom'] != ""){
	...
	if(isset($_GET['id'])){
		$requete = "UPDATE hebergement SET nom='".addslashes($nom)."', description='".addslashes($description)."' WHERE id = '".addslashes($_GET['id'])."'";
		mysql_query($requete) or die(mysql_error());
		$id = $_GET['id'];
	}else{
		$requete = "INSERT INTO hebergement (nom, description) VALUES ('".addslashes($nom)."', '".addslashes($description)."')";
		mysql_query($requete) or die(mysql_error());
		$id = mysql_insert_id();
	}
	...
	echo "<script>document.location.href='admin.php?action=hebergement';</script>";
}else{ 
	$id = "";
	$nom = "";
	$description = "";
	...
	if(isset($_GET['id'])){
		$requete = "SELECT * FROM hebergement WHERE id = '".addslashes($_GET['id'])."'";
