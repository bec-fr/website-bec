...
if(isset($_POST['nom']) && $_POST['nom'] != ""){
	...
	if(isset($_GET['id'])){
		$requete = "UPDATE immersion_totale_ville SET nom='".addslashes($nom)."', pays='".addslashes($pays)."' WHERE id = '".addslashes($_GET['id'])."'";
		mysql_query($requete) or die(mysql_error());
		$id = $_GET['id'];
	}else{
		$requete = "INSERT INTO immersion_totale_ville (nom, pays) VALUES ('".addslashes($nom)."', '".addslashes($pays)."')";
		mysql_query($requete) or die(mysql_error());
		$id = mysql_insert_id();
	}
	...
	echo "<script>document.location.href='admin.php?action=immersion_totale_ville';</script>";
}else{ 
	$id = "";
	$nom = "";
	$pays = "";
	...
	if(isset($_GET['id'])){
		$requete = "SELECT * FROM immersion_totale_ville WHERE id = '".addslashes($_GET['id'])."'";
