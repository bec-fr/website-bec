...
if(isset($_POST['pays'])){
	...
	$query = "DELETE FROM fiche_immersion_totale_formule WHERE fiche_immersion_totale = '".addslashes($id)."'";
	mysql_query($query) or die(mysql_error());
	for($i=0 ; $i<count($formule) ; $i++){
		$query = "INSERT INTO fiche_immersion_totale_formule VALUES ('".addslashes($id)."', '".addslashes($formule[$i])."')";
		mysql_query($query) or die(mysql_error());
	}
	
	/*$query = "DELETE FROM fiche_etudiant_adulte_examen WHERE fiche_etudiant_adulte = '".addslashes($id)."'";
	mysql_query($query) or die(mysql_error());
	for($i=0 ; $i<count($examen) ; $i++){
		$query = "INSERT INTO fiche_etudiant_adulte_examen VALUES ('".addslashes($id)."', '".addslashes($examen[$i])."')";
		mysql_query($query) or die(mysql_error());
	}*/
	
	/*$query = "DELETE FROM fiche_etudiant_adulte_accreditation WHERE fiche_etudiant_adulte = '".addslashes($id)."'";
	mysql_query($query) or die(mysql_error());
	for($i=0 ; $i<count($accreditation) ; $i++){
		$query = "INSERT INTO fiche_etudiant_adulte_accreditation VALUES ('".addslashes($id)."', '".addslashes($accreditation[$i])."')";
		mysql_query($query) or die(mysql_error());
	}*/
	...
	if(isset($_GET['id'])){
		...
		$query2 = "SELECT * FROM fiche_immersion_totale_formule WHERE fiche_immersion_totale = '".addslashes($_GET['id'])."'";
		...
		/*$query2 = "SELECT * FROM fiche_immersion_totale_examen WHERE fiche_immersion_totale = '".addslashes($_GET['id'])."'";
		$exec2 = mysql_query($query2) or die(mysql_error());
		$i=0;
		while($data2 = mysql_fetch_array($exec2)){
		...
		/*$query2 = "SELECT * FROM fiche_immersion_totale_accreditation WHERE fiche_immersion_totale = '".addslashes($_GET['id'])."'";
		$exec2 = mysql_query($query2) or die(mysql_error());
		$i=0;
		while($data2 = mysql_fetch_array($exec2)){
...
<table width="100%"><tr><td align="center" valign="middle"><?=(file_exists("../imagesUp/fiche_immersion_totale/".$id."-gen.jpg") ? "<img src='../imagesUp/fiche_immersion_totale/".$id."-gen.jpg' /><br /><a href='#' onclick='if(confirm(\"Voulez vous supprimer cette photo ?\")) document.location.href=\"fiche_immersion_totale-supp_photo.php?f=".$id."-gen\";'>supprimer ?</a>" : "")?></td></tr></table><br />
...
<?
$query = "SELECT * FROM immersion_totale_ville ORDER BY nom";
$exec = mysql_query($query) or die(mysql_error());
while($data = mysql_fetch_assoc($exec)){
  echo "<option value='".$data["id"]."'";
  if($data["id"] == $ville)
...
<?
$query = "SELECT * FROM formule_immersion_totale ORDER BY nom";
$exec = mysql_query($query) or die(mysql_error());
while($data = mysql_fetch_assoc($exec)){
  echo "<option value='".$data["id"]."'";
  for($i=0 ; $i<count($formule) ; $i++){
...
<TR><TD align=right valign="top">sdqsdq</TD><TD></TD><td width="20"></td><TD align=right valign="top">Accréditations&nbsp;:<br /><br />ctrl + clic</TD><TD><select name="accreditation[]" size="6" multiple="multiple">
<?
...
<tr><th>Num</th><th>Nom</th><th>Description</th></tr>
<tr align="center"><td>1</td><td><INPUT TYPE="text" size="20" name="hebergement_nom1" maxlength="255" value="<?=$hebergement_nom1?>"></td><td><textarea name="hebergement_texte1" cols="40" rows="4"><?=$hebergement_texte1?></textarea></td></tr>
