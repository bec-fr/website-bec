<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%" class="tableau">
	<tr>
		<td class="titre2">Examen - <? echo (isset($_GET['id'])? "Modification" : "Ajout"); ?></td>
	</tr>
	<tr>
		<td>
<br>
...
if(isset($_POST['nom']) && $_POST['nom'] != ""){
	...
	if(isset($_GET['id'])){
		$requete = "UPDATE examen SET nom='".addslashes($nom)."', description='".addslashes($description)."' WHERE id = '".addslashes($_GET['id'])."'";
		mysql_query($requete) or die(mysql_error());
		$id = $_GET['id'];
	}else{
		$requete = "INSERT INTO examen (nom, description) VALUES ('".addslashes($nom)."', '".addslashes($description)."')";
		mysql_query($requete) or die(mysql_error());
		$id = mysql_insert_id();
	}
	...
	echo "<script>document.location.href='admin.php?action=examen';</script>";
}else{ 
	$id = "";
	$nom = "";
	$description = "";
	...
	if(isset($_GET['id'])){
		$requete = "SELECT * FROM examen WHERE id = '".addslashes($_GET['id'])."'";
