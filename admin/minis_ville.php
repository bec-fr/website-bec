...
<input type="hidden" name="action" value="minis_ville" />
...
<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%" class="tableau">
	<tr class="titre2"><td>Villes</td><td align="right"><a href="admin.php?action=minis_villeAjouter" style="color:#FFFFFF">Ajouter une ville</a></td></tr>
	<tr>
		<td colspan="2">
<br />
...
function sup(id) {
 if (confirm("Etes-vous sûr de vouloir supprimer cette ville ?")) {
  document.location.href='minis_ville-sup.php?n='+id;
 }
}
</script>
...
$query = "SELECT v.*, p.nom as pays FROM minis_ville v LEFT OUTER JOIN minis_pays p ON v.pays = p.id WHERE 1";
...
while($row = mysql_fetch_array($exec))
{
	...
	<td class="cellule_basdroite"><a href="admin.php?action=minis_villeModif&id=<?=$row[0]?>"><img src="../adminImg/crayon.gif" border="0" /></a></td>
...
for($i=0 ; $i<ceil($nb/50) ; $i++)
{
	...
	echo ($i==$page ? "<b>".($i+1)."</b>" : "<a href='admin.php?action=minis_ville&page=$i".$plus_link."'>".($i+1)."</a>");
}
?>
</p>
