...
while($data = mysql_fetch_assoc($exec)){
	?>
	<tr>
	  <td><a href="recherche.php?site=etudiant&pays=<?=$data["id"]?>" class="tabViolet" title="Séjours linguistiques <?=stripslashes($data["nom"])?>"><strong><span class="texteRose">[ </span></strong><?=stripslashes($data["nom"])?></a></td>
	</tr>
	<?
}
?>
