...
function verifForm(){
	...
	if (a == 0) {
		document.form.action = 'junior_reserver_sejour_cro_2.php?fiche=<?=$_GET["fiche"]?>&date=<?=$_GET["date"]?>';
                                ...
                                <tr>
                                  <td align="center"><table width="98%" border="0" align="center" cellpadding="5" cellspacing="0" class="texteBlanc">
                                    <tr>
									  ...
									  ?>
		                                      <tr>
		                                      <td><input type="checkbox" name="option_transport<?=$i?>" id="option_transport<?=$i?>" value="<?=$data_dd["rid_option_transport".$i]?>" prix="<?=$data2["prix"]?>" onClick="coche_option_transport('<?=$i?>'); maj_prix();" <?=(($num_option_transport == $data_dd["rid_option_transport".$i]) ? " checked" : "")?> /> &nbsp;<?=stripslashes($data2["nom"])?><br />
                                      ...
                                      ?>
                                      			...
                                      			<td align="right"> <?=parsePrix($prixt)?> €</td>
                                      			</tr>
                                      <? 
                                  		} 
                                  	  }
									  ...
									  for($k=1 ; $k<=5 ; $k++){
										  if($data_d["option_type".$k] == "activité"){
                                      ...
                                      <tr><td height="5"></td></tr>
                                      <? for($l=1 ; $l<=5 ; $l++){ ?>
									  <? if($data_d["option_type".$l] == "activité"){// && $data_d["option_prix".$l] != "0" ?>
                                      <tr><td><input type="checkbox" name="option_activite<?=$l?>" id="option_activite<?=$l?>" value="<?=$l?>" prix="<?=$data_d["option_prix".$l]?>" onClick="coche_option_activite('<?=$l?>'); maj_prix();" <?=(($num_option_activite == $l) ? " checked" : "")?> /> &nbsp;<?=stripslashes($data_d["option_nom".$l])?></td><td align="right"><?=parsePrix($data_d["option_prix".$l])?> €</td></tr>
                                      <? } ?>
                                      <? } ?>
                                      <?
									  $aff_option_hebergement = false;
									  for($m=1 ; $m<=5 ; $m++){
										  if($data_d["option_type".$m] == "hébergement"){
                                      ...
                                      <tr><td height="5"></td></tr>
                                      <? for($n=1 ; $n<=5 ; $n++){ ?>
									  <? if($data_d["option_type".$n] == "hébergement"){// && $data_d["option_prix".$n] != "0" ?>
                                      <tr><td><input type="checkbox" name="option_hebergement<?=$n?>" id="option_hebergement<?=$n?>" value="<?=$n?>" prix="<?=$data_d["option_prix".$n]?>" onClick="coche_option_hebergement('<?=$n?>'); maj_prix();" <?=(($num_option_hebergement == $n) ? " checked" : "")?> /> &nbsp;<?=stripslashes($data_d["option_nom".$n])?></td><td align="right"><?=parsePrix($data_d["option_prix".$n])?> €</td></tr>
                                      <? } ?>
                                      <? } ?>
                                      <? if($aff_option_hebergement == true){ ?>
